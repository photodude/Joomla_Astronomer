...
<form action="<?php echo JRoute::_('index.php?option=com_astronomer&view=observations'); ?>" method="post"
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) : ?>
					...
					<td class="center">
	<a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canChange) ? JRoute::_('index.php?option=com_astronomer&task=submit.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
	<?php if ($item->state == 1): ?>
	...
	<?php endif; ?>
				...
				<?php endif; ?>
				<a href="<?php echo JRoute::_('index.php?option=com_astronomer&view=submitform&id='.(int) $item->id); ?>">
				...
				<td>
					...
					<td class="center">
						<?php if ($canEdit): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_astronomer&task=submitform.edit&id=' . $item->id, false, 2); ?>" class="btn btn-mini" type="button"><i class="icon-edit" ></i></a>
						<?php endif; ?>
						<?php if ($canDelete): ?>
							<a href="<?php echo JRoute::_('index.php?option=com_astronomer&task=submitform.remove&id=' . $item->id, false, 2); ?>" class="btn btn-mini delete-button" type="button"><i class="icon-trash" ></i></a>
						<?php endif; ?>
					</td>
				<?php endif; ?>
	...
	<?php if ($canCreate) : ?>
		<a href="<?php echo JRoute::_('index.php?option=com_astronomer&task=submitform.edit&id=0', false, 2); ?>"
