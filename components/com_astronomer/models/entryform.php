 ...
 * @since  1.6
 */
class AstronomerModelEntryForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		{
			$id = JFactory::getApplication()->getUserState('com_astronomer.edit.entry.id');
		}
		else
		{
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_astronomer.edit.entry.id', $id);
		}
		...
		{
			$this->setState('entry.id', $params_array['item_id']);
		}
	 ...
	 */
	public function getTable($type = 'Entry', $prefix = 'AstronomerTable', $config = array())
	{
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('entry.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_astronomer.entry', 'entryform', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_astronomer.edit.entry.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('entry.id');
