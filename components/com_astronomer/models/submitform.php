 ...
 * @since  1.6
 */
class AstronomerModelSubmitForm extends JModelForm
{
	private $item = null;
	...
	protected function populateState()
	{
		...
		{
			$id = JFactory::getApplication()->getUserState('com_astronomer.edit.submit.id');
		}
		else
		{
			$id = JFactory::getApplication()->input->get('id');
			JFactory::getApplication()->setUserState('com_astronomer.edit.submit.id', $id);
		}

		$this->setState('submit.id', $id);
		...
		{
			$this->setState('submit.id', $params_array['item_id']);
		}
	 ...
	 */
	public function getTable($type = 'Submit', $prefix = 'AstronomerTable', $config = array())
	{
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int) $this->getState('submit.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_astronomer.submit', 'submitform', array(
			'control'   => 'jform',
			'load_data' => $loadData
			)
		);
	...
	{
		$data = JFactory::getApplication()->getUserState('com_astronomer.edit.submit.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('submit.id');
